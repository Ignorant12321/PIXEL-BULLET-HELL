(function (PSD) {
  ...
  function drawPixelSprite(ctx, icon, x, y, scale, alpha) {
    const pattern = PSD.data.icons[icon] || PSD.data.icons.unknown;
    const s = scale || 3;
    ctx.save();
  ...
  function drawEnemy(ctx, enemy) {
    const data = PSD.data.enemies[enemy.type] || PSD.data.enemies.raider;
  ...
  function drawPickup(ctx, pickup, state) {
    const data = PSD.data.pickups[pickup.type] || PSD.data.pickups.credits;
  ...
  function drawFooter(ctx, state) {
    const waveIdx = Math.min(state.waveIndex, PSD.data.waves.length - 1);
    const wave = PSD.data.waves[waveIdx];
    const progress = state.total ? U.clamp(state.defeated / state.total, 0, 1) : (state.done / PSD.data.waves.length);
    ...
    ctx.fillText(String(waveIdx + 1).padStart(2, '0') + '/' + PSD.data.waves.length + ' ' + wave.kind, state.w - 16, y - 11);
  ...
  PSD.render.draw = function draw(ctx, state) {
    ctx.save();
