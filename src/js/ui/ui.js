...
export function createUI(game, data, U) {
  ...
  const st = {
    shopTab: 'weapon', codexTab: 'all', weaponTab: 'cannon',
  ...
  function renderShipInfo(view) {
    ...
    e.shipInfo.innerHTML = [
      ...
      '<div class="ship-icon">', U.icon(ship.icon, data.icons), '</div>',
      '<div class="ship-copy">',
  ...
  function buildWeaponOptions(view) {
    ...
    if (game.level('missile') > 0) {
      list.push({ id: 'missile', name: '微型导弹舱', icon: 'missile', tag: 'Lv ' + game.level('missile'), state: '追踪打击 · 射程联动主武器', desc: '定时发射追踪导弹，对中远距离目标进行压制。' });
    }
    if (game.level('drone') > 0) {
      list.push({ id: 'drone', name: '僚机中枢', icon: 'drone', tag: 'Lv ' + game.level('drone'), state: game.level('drone') >= 2 ? '双僚机支援' : '单僚机支援', desc: '部署僚机进行辅助射击，适合增强持续火力覆盖。' });
    }
    list.push({ id: 'bomb', name: '歼灭弹', icon: 'bomb', tag: '剩余 ' + view.bombs, state: '手动释放 · 快捷键 X', desc: '清除局部弹幕，并对范围内敌人造成伤害与 EMP 压制。' });
    return list;
  }
  ...
  function renderWeaponPanel(view) {
    ...
    if (!weapons.some(function (item) { return item.id === st.weaponTab; })) st.weaponTab = weapons[0].id;
    const active = weapons.find(function (item) { return item.id === st.weaponTab; }) || weapons[0];
    ...
    e.weaponSwitch.querySelectorAll('[data-weapon]').forEach(function (btn) {
      btn.onclick = function () { st.weaponTab = btn.dataset.weapon; renderWeaponPanel(game.view()); };
    });
  }
  ...
  function renderShop() {
    ...
    e.shopGrid.innerHTML = items.map(function (item) {
      ...
      const lvText = item.type === 'service' ? '即时补给' : item.type === 'blind' ? '随机回报' : 'Lv ' + lv + '/' + item.max;
      return [
        '<article class="item">',
